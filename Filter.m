...
starting_one=[1];
alpha_value=[0.8];
echo_filter=cat(2,starting_one,zeroes_matrix,f); % It is a way of creating a discrete time domain echo filter (refer the image is attached in the main repository)  
output=conv(y,echo_filter); %applying filter on the audio file
sound(output,fs)
numerator=echo_filter; %creating numerator of coefficients of transfer function of echo filter
d=zeros(1,R);
denominator=cat(2,starting_one,d); %creating denominator of coefficients of transfer function of echo filter
